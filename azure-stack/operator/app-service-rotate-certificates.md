...
* Certificats utilisés par Azure App Service sur Azure Stack Hub pour sécuriser les points de terminaison et la rotation du certificat d’application d’identité dans Azure Active Directory (Azure AD) ou les services ADFS (Active Directory Federation Services)
...
Lors de la rotation du certificat d’application d’identité, l’application correspondante dans AAD ou ADFS doit également être mise à jour avec le nouveau certificat.
...
1. Accédez au **portail d’administration Azure Stack Hub**
...
1. Notez la valeur **ID de l’application**, qui correspond à l’ID de l’application d’identité qui doit être mise à jour dans Azure Active Directory.

Pour effectuer la rotation du certificat pour l’application dans Active Directory, procédez comme suit :
...
1. Recherchez l’**ID d’application** et spécifiez l’ID de l’application d’identité

1. Sélectionnez l’application, puis accédez à **Certificats et secrets**

1. Cliquez sur **Charger le certificat** et chargez le nouveau certificat pour l’application d’identité avec l’un des types de fichiers suivants : .cer, .pem, .crt
...
### <a name="rotate-certificate-for-adfs-identity-application"></a>Effectuer la rotation du certificat pour l’application d’identité ADFS
...
1. Notez la valeur **ID de l’objet**, qui correspond à l’ID du principal de service qui doit être mis à jour dans ADFS.

Pour effectuer la rotation du certificat pour l’application dans ADFS, vous devez avoir accès au point de terminaison privilégié (PEP), puis mettre à jour les informations d’identification de certificat à l’aide de PowerShell, en remplaçant les espaces réservés suivants par vos propres valeurs :
...
1. Indiquez un **nouveau nom d’utilisateur administrateur local**, un nouveau **mot de passe**, confirmez ce **mot de passe**, puis cliquez sur **OK**
