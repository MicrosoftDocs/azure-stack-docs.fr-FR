...
Vous commencez par créer une inscription d’application dans votre annuaire, ce qui a pour effet de créer un [objet principal de service](/azure/active-directory/develop/developer-glossary#service-principal-object) associé pour représenter l’identité de l’application dans l’annuaire. 

Cet article commence par le processus de création et de gestion d’un principal de service, en fonction de l’annuaire que vous avez choisi pour votre instance Azure Stack Hub :
...
Dans cette section, vous inscrivez votre application en utilisant le portail Azure qui crée l’objet principal de service dans votre locataire Azure AD. Dans cet exemple, vous précisez des informations d’identification de secret client, mais le portail prend également en charge des informations d’identification basées sur un certificat X509.
...
Si vous avez déployé Azure Stack Hub avec AD FS en tant que service de gestion des identités, vous devez utiliser PowerShell pour gérer l’identité de votre application. Des exemples sont fournis ci-dessous pour la gestion des informations d’identification du principal de service, qui montrent un certificat X509 et une clé secrète client.
...
Lors de la création des informations d’identification d’un certificat, les conditions suivantes doivent être remplies :
 ...
 - Pour la production, le certificat doit être émis par une autorité de certification interne ou une autorité de certification publique. Lors de l’utilisation d’une autorité publique, vous devez inclure cette autorité dans l’image du système d’exploitation de base, dans le cadre du projet Microsoft Trusted Root Authority Program. La liste complète est disponible dans l’article [Programme de certification racine approuvé Microsoft : participants](https://gallery.technet.microsoft.com/Trusted-Root-Certificate-123665ca). Un exemple de création de certificat de test « auto-signé » s’affichera plus tard pendant la [mise à jour des informations d’identification d’un certificat](#update-a-certificate-credential). 
...
1. Ouvrez une session Windows PowerShell avec élévation de privilèges, puis exécutez le script suivant.
...
2. Une fois l’exécution du script terminée, les informations sur l’inscription de l’application s’affichent, incluant les informations d’identification du principal de service. Les `ClientID` et les `Thumbprint` sont authentifiés, puis autorisés ultérieurement à accéder aux ressources gérées par Azure Resource Manager.
...
### <a name="update-a-certificate-credential"></a>Mettre à jour les informations d’identification d’un certificat
...
2. Une fois l’exécution du script terminée, les informations sur l’inscription de l’application s’affichent, incluant les informations d’identification du principal de service. Les `ClientID` et les `ClientSecret` sont authentifiés, puis autorisés ultérieurement à accéder aux ressources gérées par Azure Resource Manager.
