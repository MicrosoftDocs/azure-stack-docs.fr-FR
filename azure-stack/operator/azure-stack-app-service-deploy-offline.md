...
Pour déployer App Service dans un environnement hors connexion, créez d’abord un package d’installation hors connexion sur une machine connectée à Internet.
...
3. Le programme d’installation App Service crée un package d’installation hors connexion et affiche le chemin pour y accéder. Vous pouvez sélectionner **Ouvrir le dossier** pour ouvrir le dossier dans l’Explorateur de fichiers.
...
1. Vérifiez que les informations de configuration du cloud App Service sont correctes. Si vous avez utilisé les paramètres par défaut durant le déploiement de l’ASDK, vous pouvez accepter les valeurs par défaut. Cependant, si vous avez personnalisé les options lors du déploiement d’Azure Stack, ou que vous effectuez le déploiement sur un système intégré, vous devez modifier les valeurs dans cette fenêtre de façon à refléter ces modifications. Par exemple, si vous utilisez le suffixe de domaine mycloud.com, vous devez remplacer votre point de terminaison Azure Resource Manager de locataire Azure Stack par `management.<region>.mycloud.com`. Après avoir vérifié vos informations, sélectionnez **Suivant**.

    ![Configurer le cloud Azure App Service dans le programme d’installation App Service][3]

1. Sur la page suivante :

   a. Sélectionnez le bouton **Se connecter** à côté de la zone **Abonnements Azure Stack**. 

   b. Indiquez votre compte Administrateur. Par exemple : cloudadmin@azurestack.local. Entrez votre mot de passe, puis sélectionnez **Se connecter**.

   c. Dans la zone **Abonnements Azure Stack**, sélectionnez **Abonnement du fournisseur par défaut**.
   > [!NOTE]
   > App Service peut être déployé seulement sur l’**abonnement du fournisseur par défaut**.

   d. Dans la zone **Emplacements Azure Stack**, sélectionnez l’emplacement qui correspond à la région où vous effectuez le déploiement. Par exemple, si vous déployez sur ASDK, sélectionnez **local**.

   e. Sélectionnez **Suivant**.

      ![Abonnements et emplacements Azure Stack dans le programme d’installation App Service][4]

1. Vous pouvez autoriser le programme d’installation d’App Service à créer un réseau virtuel et des sous-réseaux associés. Vous pouvez aussi déployer sur un réseau virtuel existant, comme ce qui est configuré via [ces étapes](azure-stack-app-service-before-you-get-started.md#virtual-network).
   - Pour utiliser la méthode du programme d’installation d’App Service, sélectionnez **Créer un réseau virtuel avec les paramètres par défaut**, acceptez les valeurs par défaut, puis sélectionnez **Suivant**.
...
1. Entrez les détails SQL Server de l’instance de serveur utilisée pour héberger les bases de données du fournisseur de ressources App Service, puis sélectionnez **Suivant**. Le programme d’installation valide les propriétés de connexion SQL. Pour le nom du serveur SQL Server, vous **devez** entrer l’adresse IP interne ou le nom de domaine complet (FQDN).
    ...
    >
    > À compter d’Azure App Service sur Azure Stack 1.3 et ultérieur, le programme d’installation vérifie si l’ordinateur exécutant SQL Server a l’autonomie de la base de données activée au niveau de SQL Server. Si ce n’est pas le cas, l’exception suivante s’affiche :
    > ```sql
    ...
    >    ***********************************************************
    > ```
    > Pour plus d’informations, consultez les [notes de publication pour Azure App Service sur Azure Stack 1.3](azure-stack-app-service-release-notes-update-three.md).
...
1. Dans la zone **Sélectionner l’image de plateforme**, choisissez votre image de machine virtuelle Windows Server 2016 de déploiement parmi les images disponibles dans le fournisseur de ressources de calcul pour le cloud App Service. Sélectionnez **Suivant**.
...
1. Dans le portail utilisateur Azure Stack, sélectionnez **Créer une ressource** > **Web + Mobile** > **Application web**.
