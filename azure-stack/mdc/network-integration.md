---
title: Intégration réseau MDC
description: En savoir plus sur l’intégration réseau Azure Stack pour l’appareil MDC.
author: justinha
ms.author: justinha
...
L’utilisation d’un protocole de routage dynamique comme BGP garantit que votre système est toujours informé des changements de réseau et facilite l’administration. Pour une meilleure sécurité, un mot de passe peut être défini sur le peering BGP entre la périphérie et la bordure.

Comme indiqué dans le diagramme suivant, la publication de l’espace d’adressage IP privé sur le commutateur TOR est bloquée au moyen d’une liste de préfixes. Celle-ci refuse la publication du réseau privé et est appliquée comme carte de routage sur la connexion entre le ToR et la bordure.
...
Pour intégrer Azure Stack dans votre environnement réseau à l’aide d’un routage statique, les quatre liens physiques entre la bordure et le périphérique doivent être connectés. La haute disponibilité ne peut pas être garantie en raison du mode de fonctionnement du routage statique.

Le périphérique de bordure doit être configuré avec des itinéraires statiques pointant vers les quatre adresses IP P2P définies entre le périphérique et la bordure pour le trafic destiné aux différents réseaux dans Azure Stack, mais seul le réseau des adresses IP virtuelles externe ou publique est requis pour l'opération. Des itinéraires statiques vers les réseaux BMC et externe sont requis pour le déploiement initial. Les opérateurs peuvent choisir de laisser des itinéraires statiques à la bordure pour accéder à des ressources d’administration qui se trouvent sur le réseau BMC et de l’infrastructure. L’ajout de routes statiques aux réseaux de l’infrastructure des commutateurs et de la gestion des commutateurs  est facultatif.
...
Le routage statique s’applique seulement aux liaisons montantes entre le périphérique et les commutateurs de bordure.
...
Les adresses IP du redirecteur DNS sont définies pendant le déploiement d’Azure Stack. Cependant, si les adresses IP du redirecteur doivent être mises à jour pour une quelconque raison, vous pouvez modifier les valeurs en vous connectant au point de terminaison privilégié et en exécutant les cmdlets  *Get-AzSDnsForwarder*  et  *Set-AzSDnsForwarder [[-IPAddress]\<IPAddress[]\>]*   PowerShell. Pour plus d’informations, voir le point de terminaison privilégié.
...
Un ensemble d’adresses IP virtuelles d’infrastructure est nécessaire pour la publication des points de terminaison Azure Stack sur des réseaux externes. La table  *Point de terminaison (adresse IP virtuelle)*   affiche chaque point de terminaison, le port requis et le protocole. Consultez la documentation de déploiement spécifique au fournisseur de ressources pour les points de terminaison nécessitant des fournisseurs de ressources supplémentaires, comme le fournisseur de ressources SQL.
